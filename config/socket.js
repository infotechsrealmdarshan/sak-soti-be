...
export const initializeSocket = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    logger.log(`✅ User ${userId} connected to socket`);
    ...
    (async () => {
      try {
        ...
        logger.error("⚠️ Failed to auto-join chat rooms:", error.message);
      }
    })();
    ...
    socket.on("joinChat", (data) => {
      ...
      logger.log(`✅ User ${userId} joined ${roomName}`);
    });
    ...
    socket.on("leaveChat", (chatId) => {
      if (chatId) {
        ...
        logger.log(`👤 User ${userId} left chat room: chat:${chatId}`);
      }
    });
    ...
    socket.on("sendMessage", async (data) => {
      try {
        ...
        logger.log("📤 Backend received sendMessage:", { chatId, message, senderId });
        ...
        logger.log(`✅ Message broadcasted to chat:${chatId} from user:${senderId}`);
    ...
    socket.on("getMessages", async (data) => {
      try {
        ...
        logger.log("📥 Backend received getMessages:", { chatId, page, limit });
        ...
        logger.error("❌ Error in getMessages handler:", error);
    ...
    socket.on("markMessagesAsRead", async (data) => {
      ...
      if (chatId && mongoose.Types.ObjectId.isValid(chatId)) {
        try {
          ...
          logger.log(`📖 User ${userId} marked messages as read in chat: ${chatId}`);
        } catch (error) {
    ...
    socket.on("typing:start", async (data) => {
      const { chatId } = data;
      logger.log("⌨️ [BACKEND] TYPING START RECEIVED:", {
        chatId: chatId,
      ...
      if (!chatId) {
        logger.error("❌ [BACKEND] No chatId in typing:start");
        return;
      }
      ...
      try {
        ...
        logger.log("📤 [BACKEND] Broadcasting typing start to room:", {
          chatId: chatId,
          userId: socket.userId,
        ...
        logger.error("❌ [BACKEND] typing:start error:", error.message);
      }
    });

    socket.on("typing:stop", async (data) => {
      const { chatId } = data;
      logger.log("🛑 [BACKEND] TYPING STOP RECEIVED:", {
        chatId: chatId,
        userId: socket.userId,
        socketId: socket.id,
        timestamp: new Date().toISOString()
      ...
      logger.log("📤 [BACKEND] Broadcasting typing stop to room:", {
        chatId: chatId,
        userId: socket.userId,
        targetRoom: `chat:${chatId}`
      });
      ...
      try {
        ...
        logger.error("❌ [BACKEND] typing:stop broadcast error:", error.message);
      }
    });

    // ✅ NEW: Handle connection confirmation
    socket.on("connected", (data) => {
      logger.log("✅ Client confirmed connection:", data);
      socket.emit("connected", {
    ...
    socket.on("disconnect", (reason) => {
      logger.log(`❌ User ${userId} disconnected from socket. Reason:`, reason);
    });
  });

  logger.log("✅ Socket.IO initialized with enhanced event handlers");
  return io;
};
